import { LazyServiceIdentifier } from '../annotation/inject';
import { resolveForwardRef } from '../annotation/forward-ref';
...
function getDependencies(metadataReader: MetadataReader, func: Function): Target[] {
  const constructorName = getFunctionName(func);
  const targets: Target[] = getTargets(metadataReader, constructorName, func, false);
  return targets;
}

function getTargets(metadataReader: MetadataReader, constructorName: string, func: Function, isBaseClass: boolean): Target[] {
...
function getConstructorArgsAsTarget(index: number,
                                    isBaseClass: boolean,
  ...
  const injectIdentifier = resolveForwardRef(metadata.inject || metadata.multiInject);
...
function getConstructorArgsAsTargets(isBaseClass: boolean,
                                     constructorName: string,
                                     serviceIdentifiers: any,
                                     constructorArgsMetadata: any,
                                     iterations: number) {
...
function getClassPropsAsTargets(metadataReader: MetadataReader, constructorFunc: Function) {
...
function getBaseClassDependencyCount(metadataReader: MetadataReader, func: Function): number {
  ...
  if (baseConstructor !== Object) {
    ...
    const metadata: any[] = targets.map((t: Target) =>
      t.metadata.filter((m: Metadata) =>
...
function formatTargetMetadata(targetMetadata: any[]) {
  ...
  return {
    inject     : targetMetadataMap[MetadataKeys.INJECT_TAG],
    multiInject: targetMetadataMap[MetadataKeys.MULTI_INJECT_TAG],
    targetName : targetMetadataMap[MetadataKeys.NAME_TAG],
    unmanaged  : targetMetadataMap[MetadataKeys.UNMANAGED_TAG]
  };
